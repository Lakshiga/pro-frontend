...
const Event = () => {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#121212', }}>
        <div style={{ border: '4px solid rgba(204, 255, 0, 0.1)', borderLeftColor: '#ccff00',  borderRadius: '50%',  width: '40px',  height: '40px',  animation: 'spin 1s linear infinite', }}></div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', justifyContent: 'center',alignItems: 'center', backgroundColor: '#121212', color: '#ff6b6b', fontSize: '1.2rem', }}>
        {error}
      </div>
    );
  }

  const containerStyle = { minHeight: '100vh', padding: '2rem', backgroundColor: '#121212', fontFamily: 'Arial, sans-serif',color: '#ffffff',};

  const contentStyle = { maxWidth: '1200px', margin: '0 auto', padding: '2rem', backgroundColor: '#1e1e1e', borderRadius: '1rem', boxShadow: '0 0 30px rgba(204, 255, 0, 0.2)', };

  const headerStyle = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem',};

  const logoTitleStyle = { display: 'flex',  alignItems: 'center', };

  const logoStyle = { width: '80px', height: '80px', marginRight: '1rem', };

  const titleStyle = { color: '#ccff00', margin: 0, fontSize: '2.5rem', fontWeight: 'bold', };

  const subtitleStyle = { color: '#d4d4d4', margin: '0.5rem 0 0 0', fontSize: '1.2rem', };

  const backButtonStyle = { backgroundColor: '#ccff00', color: 'black', padding: '0.75rem 1.5rem', borderRadius: '0.5rem', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', transition: 'all 0.3s ease', boxShadow: '0 0 10px rgba(204, 255, 0, 0.5)',};

  const infoCardsStyle = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem', marginBottom: '3rem', };
      ...
      <div style={contentStyle}>
        <header style={headerStyle}>
          <div style={logoTitleStyle}>
            <img src={logo} alt="Match Master Logo" style={logoStyle} />
            <div>
        ...
        {event ? (
          <>
            ...
            <Section title="Organizer" icon={<FaUserTie />}>
              {event.organizer_id ? (
                <p style={{  color: '#ccff00',  fontSize: '1.1rem',  backgroundColor: '#2a2a2a',  padding: '1rem',  borderRadius: '0.5rem'
              ...
              {umpireDetails && umpireDetails.length > 0 ? (
                <ul style={{  listStyleType: 'none',  padding: 0,  display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
                  {umpireDetails.map(umpire => (
                    <li key={umpire.id} style={{backgroundColor: '#2a2a2a',padding: '1rem',borderRadius: '0.5rem',color: '#ccff00',fontSize: '1.1rem',transition: 'all 0.3s ease',boxShadow: '0 0 10px rgba(204, 255, 0, 0.2)',
                    }}
                    onMouseOver={(e) => {
                      e.currentTarget.style.transform = 'scale(1.05)';
                ...
                <>
                  <ul style={{listStyleType: 'none', padding: 0, display: 'grid',  gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))',  gap: '1rem',}}>
                    {playersDetails.map(player => (
                      <li key={player.id} style={{ backgroundColor: '#2a2a2a', padding: '1rem', borderRadius: '0.5rem', color: '#ccff00', fontSize: '1.1rem', transition: 'all 0.3s ease', boxShadow: '0 0 10px rgba(204, 255, 0, 0.2)',
                      }}
                      onMouseOver={(e) => {
                        e.currentTarget.style.transform = 'scale(1.05)';
                        e.currentTarget.style.boxShadow = '0 0 20px rgba(204, 255, 0, 0.4)';
                  ...
                  <button
                    ...
                    style={{ backgroundColor: '#ccff00',color: 'black',padding: '0.75rem 1.5rem', border: 'none', borderRadius: '0.5rem', cursor: 'pointer', marginTop: '1rem', fontWeight: 'bold', transition: 'all 0.3s ease', boxShadow: '0 0 10px rgba(204, 255, 0, 0.5)',
                    }}
                    onMouseOver={(e) => {
                      e.currentTarget.style.transform = 'scale(1.05)';
                      e.currentTarget.style.boxShadow = '0 0 20px rgba(204, 255, 0, 0.8)';
                ...
                <ul style={{ listStyleType: 'none', padding: 0 }}>
                  {matches.map((match) => (
                    match && match._id ? (
                      <li
                        key={match._id}
                        style={{ backgroundColor: '#2a2a2a', padding: '1rem', borderRadius: '0.5rem',
                          marginBottom: '1rem',
                          color: '#ccff00',
                          display: 'flex',
